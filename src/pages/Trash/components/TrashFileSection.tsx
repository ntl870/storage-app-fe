...
import { Folder, File, useRestoreFileFromTrashMutation, useDeleteFileMutation } from '@generated/schemas';
...
export const TrashFileSection = ({ files, handleClickItem, selectedItem, refetch }: Props) => {
      ...
      {files.map((file) => (
        <Dropdown menu={{ items: getItems(file) }} key={file.ID} trigger={['contextMenu']}>
          <Col className="m-4">
            ...
            >
              <ItemCardContent className={`${selectedItem?.ID === file.ID ? 'bg-blue-100' : ''}`}>
